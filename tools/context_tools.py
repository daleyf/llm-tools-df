"""Grep-like utilities to pull text snippets into LLM prompts."""
...
# File extensions that are treated as plain text without reading bytes.
DEFAULT_TEXT_EXTS = {
...
def _estimate_tokens(text: str) -> int:
    ...
    Attempts to use ``tiktoken`` if available, otherwise falls back to a simple
...
def _iter_files(paths: Iterable[str], include: List[str], exclude: List[str], max_bytes: int) -> Iterable[Path]:
    ...
    for raw in paths:
        # Expand simple globs ourselves. Path.glob handles patterns in the last component only.
        if any(ch in raw for ch in "*?[]"):
            candidates = Path().glob(raw)
        else:
            candidates = [Path(raw)]
        for p in candidates:
            if p.is_dir():
